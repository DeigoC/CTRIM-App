...
class _ViewImageVideoState extends State<ViewImageVideo> {
  ...
  Widget _createVideoPage(String src){
    ...
    if(thisController.value.initialized){
      if(_orientation == Orientation.portrait) return _createPortraitVideoPage(thisController);
      else return _createLandscapeVideoPage(thisController);
    }
    ...
    return Row(
      ...
      children: [
        IconButton(
         ...
         IconButton(
          icon: Icon(controller.value.isPlaying ? Icons.pause : Icons.play_arrow, size: 40,),
          onPressed: (){
            setState(() {
