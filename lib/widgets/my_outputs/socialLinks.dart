...
class SocialLinksDisplay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (socialLinks.length == 0)
      return Center(child: Text('No Social Links/Contacts added.'));
    return Align(
  ...
  void _iconClick(String type, BuildContext context) async {
    ...
    if (type.compareTo('Email') == 0) {
      ...
      showDialog(
          ...
          builder: (_) {
            return AlertDialog(
              ...
              content: Text(
                "The number is: '$link'. Do you wish to open your Phone or SMS app?",
              ),
              actions: [
                MyFlatButton(
                  label: 'Cancel',
      ...
      if (await canLaunch(socialLinks[type])) {
        if (await launch(socialLinks[type],
          ...
          showDialog(
              ...
              builder: (_)=> AlertDialog(
                ...
                content: Text(
                    "Your device doesn't seem to have the app to open: '$link'\n\nDo you wish to continue with webview?"),
                actions: [
                  MyFlatButton(
                    label: 'Cancel',
                    onPressed: () {
        ...
        showDialog(
            ...
            builder:(_)=> AlertDialog(
              ...
              content: Text(
                  "The link the user provided is not in the correct format and so cannot be opened.\nLink: '$link'"),
              actions: [
                MyFlatButton(
                  label: 'Ok',
                  onPressed: () => Navigator.of(context).pop(),
...
class _SocialLinksEditState extends State<SocialLinksEdit> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: _buildBody(),
        floatingActionButton: (_adding || _editing)
            ? null
  ...
  SingleChildScrollView _buildAddingLink() {
    ...
    availableTypes.removeWhere(
        (e) => widget._adminBloc.selectedUser.socialLinks.containsKey(e));
        ...
        children: [
          ...
          MyRaisedButton(
            ...
            onPressed: _enableButton
                ? () {
                    widget._adminBloc.selectedUser
                            .socialLinks[_selectedTypeToEdit] =
                        _tecLinkContact.text.trim();
                    _backToViewList();
                  }
                : null,
          ),
  ...
  ListView _buildViewing() {
    Map<String, String> socialLinks =
        widget._adminBloc.selectedUser.socialLinks;
    List<String> types = socialLinks.keys.toList();
    types.sort();
  ...
  void _canEnableSaveButton() {
    if (_tecLinkContact.text.trim().isNotEmpty &&
        _selectedTypeToEdit != null &&
        !_enableButton) {
      setState(() {
        ...
        (_tecLinkContact.text.trim().isEmpty || _selectedTypeToEdit == null)) {
      setState(() {
        _enableButton = !_enableButton;
      });
    }
